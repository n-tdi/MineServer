package world.ntdi.api.cooldown;
...
public class Cooldown {
    ...
    private final Map<Object, Long> m_cooldowns = new ConcurrentHashMap<>();
    private final LongSupplier m_timeSupplier;
    private final TimeUnit m_precision;
    ...
    public Cooldown(final TimeUnit precision, final LongSupplier timeSupplier) {
        ...
        this.m_timeSupplier = timeSupplier;
    }
    ...
    public Cooldown(final TimeUnit precision) {
        ...
        this.m_precision = precision;
        this.m_timeSupplier = supplier;
    }

    public void removeCooldown(final Object object) {
        m_cooldowns.remove(getUid(object));
    }
    ...
    public void clearOldEntries() {
        ...
        m_cooldowns.entrySet().removeIf(entry -> now >= entry.getValue());
    }

    private long getTime() {
        return m_timeSupplier.getAsLong();
    }
    ...
    public void setCooldown(final Object object, final long duration, final TimeUnit timeUnit) {
        m_cooldowns.put(getUid(object), getTime() + m_precision.convert(duration, timeUnit));
    }
    ...
    public long getCooldownEnd(final Object object) {
        final Object uid = getUid(object);
        final long end = m_cooldowns.getOrDefault(uid, 0L);
        if (end == 0) return 0;
        if (getTime() > end) {
            return 0;
        }
    ...
    public long getCooldownRemaining(final Object object, final TimeUnit timeUnit) {
        ...
        return timeUnit.convert(endTime - getTime(), m_precision);
    }
